...
- 採用条件付き保留対象: `ecc-watchlist.md`（master ブランチ専用）
- 取り込みファイル一覧の動的取得: `Skill: ecc-sync`（master ブランチ専用、inventory モード）

ECC のライセンスが MIT 以外に変更された場合、ecc-sync 本実装（別途 spec で定義予定）が検出して停止する。それまでは MIT 前提で運用する。

## 私的情報の混入禁止

このディレクトリの汎用ファイル（LICENSE, README）は `master` および `master-upstream` 両ブランチで共有される。private 情報（API key, internal URL など）を絶対に含めない。`ecc-watchlist.md` は個人の保留判断を反映するため master のみ。
